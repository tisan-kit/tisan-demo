...
upgrade_download(void *arg, char *pusrdata, unsigned short length)
{
            ...
            (ptr = (char *)os_strstr(pusrdata, "Content-Length")) != NULL) 
    {
        ...
        MD5Update(&context, ptr + 4, length);
        ...
        if (ptr != NULL)
        {
            ...
            if (ptmp2 != NULL)
            {
                ...
                if(sumlength <180000 || sumlength > 300000)
                {
                	os_printf("not a valid upgrade file\n");
                }

                os_printf("sumlenth:%d\n",sumlength);
            }
            else
            {
    ...
    else 
    {
        ...
        MD5Update(&context, pusrdata, length);
    ...
    if (totallength == sumlength) 
    {
    	...
    	os_printf("\n");
    	if(os_memcmp(digest,((struct upgrade_server_info *)(upgrade_conn->reverse))->md5, 16) !=0)
    	{
    		UPGRADE_DBG("download file wrong.\n");
    		return;
    	}
...
/******************************************************************************
 * FunctionName : reconnect_callback
 * Description  : .process the tcp client reconnect
 * Parameters   : none
 * Returns      : none
*******************************************************************************/
...
{
    os_printf("tcp client reconnect...error:%d\n", err);
}
...
upgrade_connect(struct upgrade_server_info *server)
{
    ...
    espconn_regist_reconcb(upgrade_conn, reconnect_callback);
...
#ifdef UPGRADE_SSL_ENABLE
pando_system_upgrade_start_ssl(struct upgrade_server_info *server)
#else
pando_system_upgrade_start(struct upgrade_server_info *server)
#endif
{
    /*if (system_upgrade_flag_check() == UPGRADE_FLAG_START)
    {
